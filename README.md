...
每次工具调用在执行前依次经过五层判断：不可覆盖的危险命令黑名单、路径沙箱、分层规则、权限模式，以及必要时的用户确认。权限拒绝会回灌 Agent Loop，模型可以改用更安全的工具、命令或路径继续任务。
...
规则使用真实工具名。`run_command` 匹配完整命令，文件工具匹配规范化的工作区相对路径，`find_files` 匹配查找模式，`search_code` 匹配可选搜索范围 `path`。包含 `*`、`?` 或 `[...]` 的模式按大小写敏感 glob 匹配，其余模式精确匹配。
...
命令行 `--permission-mode` 优先于本地、项目和用户配置；所有位置都未声明时使用 `default`。default 模式询问时可选择拒绝、本次、本会话或永久放行。永久放行只向 `.mycode/permissions.local.yaml` 写入当前目标的精确 allow，不会修改项目共享或用户全局规则。
...
一次模型响应里出现多个工具调用时，Mycode 会按安全性分批：有副作用工具串行判定和执行；只读工具只有在权限判定通过后才并发执行。审批和永久规则写入会串行化，避免交互或持久化竞态。
